...
export const sharedMain = async ({ repo }: InputData<"vscode-extension">) => {
    ...
    await fs.promises.writeFile(
        hasCode ? "out/CHANGELOG.MD" : "CHANGELOG.MD",
        CHANGELOG_CONTENT,
        "utf-8",
    )
    ...
    const newReadme = await markdownRemoveHeading(
        readme,
        "Extension Development Notes",
    )
    await fs.promises.writeFile(
        hasCode ? "out/README.MD" : "README.MD",
        newReadme,
    )
    ...
    if ((await fs.promises.stat(vsixPath)).size > SIZE_LIMIT)
        throw new Error("SIZE_LIMIT exceeded in 3 MG")
    return { vsixPath }
}
