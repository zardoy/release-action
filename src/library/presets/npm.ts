...
export const main = async ({ repo, octokit }: InputData<"npm">) => {
    ...
    // important defaul
    const buildDir =
        (await readTsconfigJsonFile({ dir: "." })).compilerOptions?.outDir ??
        "build"
    ...
    for (const [fieldName, generatedValue] of Object.entries(generatedFields)) {
        ...
        if (
            generatedValue === initialPackageJson[fieldName] &&
            !alwaysRemove.has(fieldName)
        )
            continue
    ...
    if (packageJson.private)
        throw new Error(
            "Packages that are going to publish to NPM can't be private",
        )

    validatePaths(
        process.cwd(),
        await readPackageJsonFile({ dir: process.cwd() }),
    )
